...
/* The /learn/ showcase trailer. Same player mechanics as /trailer/ (rAF clock
 * walking a scene timeline, freeze-on-pause, seek-by-scene, browser-native HD
 * export) with its own timeline about the technique lessons. Fully separate
 * from /trailer/ so retiming one can never disturb the other. */
 ...
 * `/trailer2/?preview=pointing&ts=6` renders the "pointing" scene frozen at 6s
...
export default function Trailer2() {
  ...
  const exportVideo = useCallback(async () => {
    ...
    if (!nav?.getDisplayMedia || typeof MediaRecorder === "undefined") {
      alert(
        "This browser can't capture the tab. Use a desktop Chromium browser, or screen-record /trailer2/ manually.",
      );
      return;
    }
    ...
    rec.onstop = () => {
      ...
      a.href = url;
      a.download = "sudotiles-learn-trailer.webm";
      a.click();
  ...
  return (
    <div
      className={`relative h-full w-full overflow-hidden bg-[var(--bg2)] text-[color:var(--accent)] ${
        !playing && started ? "t2-paused" : ""
      } ${recording ? "t2-recording" : ""}`}
    >
      ...
      <div
        ...
        style={{ opacity: 0, animation: flash ? "t2-flash 0.22s ease-out both" : undefined }}
      />
      ...
      {!started && (
        ...
        >
          <span className="text-[clamp(2rem,8vw,5.5rem)] font-bold tracking-[-0.02em] text-[var(--t2-ember)]">
            Learn Advanced Techniques
          </span>
          <span className="flex items-center gap-3 rounded-full bg-white/[0.07] px-6 py-3 text-[clamp(1rem,2.6vw,1.4rem)] font-medium text-[var(--ink0)] shadow-[0_0_0_1px_rgba(255,255,255,0.12)_inset]">
            <span className="text-[var(--t2-ember)]">▶</span> Play trailer
          </span>
          <span className="text-[13px] text-[var(--ink4)]">with sound</span>
        </button>
      )}
  ...
  return (
    <button
      ...
      className={`flex h-10 w-10 cursor-pointer items-center justify-center rounded-full border-none text-[16px] shadow-[0_0_0_1px_rgba(255,255,255,0.1)_inset] transition-[filter,opacity] duration-100 hover:brightness-125 disabled:cursor-default disabled:opacity-30 ${
        active ? "bg-[var(--accent)] text-[var(--bg2)]" : "bg-white/[0.07] text-[var(--ink0)]"
      }`}
    >
      {children}
    </button>
